...
import { Loader2, ShoppingBag, Sparkles, Play, AlertTriangle, Camera, Mic, MicOff, Search, Globe, X, Plus, Volume2 } from 'lucide-react';
import {
  describeMeetError,
  getLocalIdentity,
  initMeetAddon,
...
export default function MeetSidePanel() {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          reader.onloadend = async () => {
            ...
            if (data.answer) {
              setUpdates(prev => [{ at: Date.now(), userSaid: 'Voice Question', aiAnswer: data.answer }, ...prev]);
              // Log RCA telemetry event
              fetch('/api/voice-logs', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ eventType: 'VOICE_QUESTION', userName: displayName, userSaid: 'Voice Question', aiAnswer: data.answer, audioByteLength: wavBlob.size })
              }).catch(() => {});
            }
    ...
    reader.onloadend = async () => {
      ...
      try {
        const res = await fetch('/api/live-call', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ frameBase64: base64Frame, userQuestion: 'Identify products and prices in this image', userName: displayName })
        });
        const data = await res.json();
        if (data.answer) {
          setUpdates(prev => [{ at: Date.now(), userSaid: 'Photo Scan', aiAnswer: data.answer }, ...prev]);
          if (data.targetLabel) {
            setCartItems(prev => [...prev, { id: `item-${Date.now()}`, name: data.targetLabel }]);
          }
          fetch('/api/voice-logs', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ eventType: 'PHOTO_SCAN', userName: displayName, userSaid: 'Photo Scan', aiAnswer: data.answer, frameByteLength: file.size })
          }).catch(() => {});
        }
      } catch(err) {}
      setIsAiProcessing(false);
  ...
  const handleMemorySearch = (e: React.FormEvent) => {
    ...
    setMemoryMatch(`Found "${memoryQuery}" in past 30-min memory (timestamp: 14 mins ago)`);
  };
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-2 shrink-0">
            ...
            <button
              ...
              className="bg-slate-900 hover:bg-slate-800 border border-teal-500/30 text-teal-300 font-bold text-xs rounded-xl px-2.5 py-2 flex items-center justify-center gap-1.5 active:scale-[0.98] transition"
            >
            ...
            <button
              ...
              disabled={isAiProcessing}
              className={`${isRecording ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-900 hover:bg-slate-800 border border-indigo-500/30 text-indigo-300'} font-bold text-xs rounded-xl px-2.5 py-2 flex items-center justify-center gap-1.5 active:scale-[0.98] transition`}
            >
          ...
          {/* AR TRANSLATION CARDS */}
          {arTranslations.length > 0 && (
            <div className="space-y-1 shrink-0">
              <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                <Globe className="w-3 h-3 text-teal-400" /> AR Translations
              </span>
              {arTranslations.map((t, idx) => (
                <div key={idx} className="bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-[11px] flex items-center justify-between">
                  <div>
                    <p className="font-bold text-white">{t.original}</p>
                    <p className="text-teal-300 text-[10px]">{t.translated}</p>
                  </div>
                  <button type="button" onClick={() => setArTranslations(prev => prev.filter((_, i) => i !== idx))}><X className="w-3 h-3 text-slate-500 hover:text-white" /></button>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto space-y-2 border-t border-slate-900 pt-2">
            {isAiProcessing && (
              <p className="text-[11px] text-teal-400 flex items-center gap-1.5 animate-pulse">
                <Loader2 className="w-3.5 h-3.5 animate-spin" /> Gemini AI processing…
              </p>
            )}
            {updates.length === 0 && !isAiProcessing ? (
              <p className="text-[11px] text-slate-500 leading-relaxed">
                Tap <strong>📸 Scan Photo</strong> or <strong>✨ Ask AI</strong> to scan products, translate labels, and search past memory!
              </p>
            ) : (
              updates.map((u) => (
                <div key={u.at} className="bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-2">
                  {u.userSaid && <p className="text-[10px] text-slate-500 font-bold">“{u.userSaid}”</p>}
